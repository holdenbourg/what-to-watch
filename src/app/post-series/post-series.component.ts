...
export class PostSeriesComponent implements OnInit {
  ...
  public currentPostingSeries: RatedSeriesModel = this.localStorageService.getInformation('currentPostSeries');
  ...
  public watchedWith: string = `${this.currentUser.firstName} watched ${this.currentPostingSeries.title} with `;
  ...
  ngOnDestroy() {
    ...
    this.localStorageService.clearInformation('currentPostSeries');
  }
  ...
  onArchive() {
    if(this.caption.length > 150) {
      ...
      let post: RawUserPostModel = {
        postId: this.currentPostingSeries.postId,
        profilePicture: this.currentUser.profilePicture,
        username: this.currentUser.username,
        poster: this.currentPostingSeries.poster,
      ...
      let currentSeriessDB: RatedSeriesModel[] = this.localStorageService.getInformation('ratedSeries');
      currentSeriessDB.push(this.currentPostingSeries);
      ...
      this.localStorageService.setInformation('ratedSeries', currentSeriessDB);
  ...
  addPostToTaggedUsersAccounts(post: RawUserPostModel) {
    ...
    taggedUsers.forEach((taggedUser) => {
      ...
      user.taggedPostIds.push(this.currentPostingSeries.postId);
